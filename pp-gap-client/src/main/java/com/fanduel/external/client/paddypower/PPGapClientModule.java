...
import com.fanduel.external.client.paddypower.impl.PPGapClientImpl;
import com.fanduel.external.client.paddypower.impl.ResilientPPGapClientImpl;
...
public class PPGapClientModule extends ConfigAwareModule {
	...
	private static final String DEFAULT_WS_NAMESPACE = "http://endpoint.web.api.games.betfair.com/";
	private static final String DEFAULT_WS_SERVICE_NAME = "PartnerAPIServiceImplService";
	private static final String DEFAULT_CLIENT_ID_HEADER = "ppfantasy";

	public PPGapClientModule( final Config config ) {
		super( config );
	}

	protected void bindPPGapClient( String injectedName ) throws MalformedURLException {
		PPGapClient ppGapClient = createBareClient( injectedName );
		...
		if ( section.getBool( RES4J_CIRCUIT_BREAKER_ENABLED, true ) ) {
			...
			ppGapClient = new ResilientPPGapClientImpl( ppGapClient, circuitBreakerRegistry,
					timeLimiterEnabled, timeLimiterExecutor, timeLimiterRegistry );

		}

		bind( PPGapClient.class ).toInstance( ppGapClient );
	}
	...
	private PPGapClient createBareClient( String injectedName ) throws MalformedURLException {
		Config section = getConfig().section( injectedName );
		...
		return new PPGapClientImpl( wsdlUrl, qName, clientId );
	}
	...
	private Executor createTimeLimiterExecutor( final String injectedName, final Config section ) {
		return new InstrumentedExecutorService( Executors.newFixedThreadPool(
				...
				name( PPGapClientModule.class, injectedName, "time-limiter-executor" ) );
	}
